...
const GENRES = [
  { name: "Fiction", emoji: "📖", color: "from-purple-100 to-purple-50" },
  { name: "Non-Fiction", emoji: "📚", color: "from-blue-100 to-blue-50" },
  { name: "Mystery", emoji: "🔍", color: "from-slate-100 to-slate-50" },
  { name: "Romance", emoji: "💕", color: "from-pink-100 to-pink-50" },
  {
    name: "Science Fiction",
    emoji: "🚀",
    color: "from-indigo-100 to-indigo-50",
  },
  { name: "Biography", emoji: "✍️", color: "from-amber-100 to-amber-50" },
  { name: "Self-Help", emoji: "🌱", color: "from-green-100 to-green-50" },
  { name: "History", emoji: "🏛️", color: "from-orange-100 to-orange-50" },
];
    ...
    <main className="min-h-screen bg-surface">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32">
          <div className="max-w-3xl">
            ...
            <p className="mt-6 text-lg text-white max-w-xl leading-relaxed">
              Discover thousands of pre-loved and new books at unbeatable
              prices. From timeless classics to today&apos;s bestsellers — your
              next favourite read is waiting.
            </p>
            <div className="flex flex-wrap gap-4 mt-10">
              <Link
                href="/shop"
                className="px-8 py-3.5 bg-amber-400 text-brand-950 font-semibold rounded-pill hover:bg-amber-300 transition-colors shadow-lg"
              >
                Browse Collection
              </Link>
              <Link
                href="/shop?bestseller=true"
                className="px-8 py-3.5 bg-white/10 border border-white/20 text-white font-medium rounded-pill hover:bg-white/20 transition-colors"
              >
                View Bestsellers
              </Link>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-3xl font-serif text-brand-950">
              Browse by Genre
            </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          {GENRES.map(({ name, emoji, color }) => (
            ...
            >
              ...
              <span className="text-xs font-medium text-brand-700 text-center leading-tight">
                {name}
              </span>
            </Link>
          ))}
      ...
      {featuredBooks.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {featuredBooks
              .slice(0, 8)
              .map((book: Parameters<typeof BookCard>[0]["book"]) => (
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center mb-12">
          ...
          <p className="text-brand-500 mt-2">
            We&apos;re not just a bookstore — we&apos;re a community of readers
          </p>
        </div>
          ...
          {[
            {
              ...
              description:
                "Pre-loved books at a fraction of the original price. Knowledge should be accessible to everyone.",
            },
            {
              icon: (
                <svg
              ...
              description:
                "Every book is reviewed for condition and accurately described. No surprises when it arrives.",
            },
            {
              icon: (
                <svg
              ...
              title: "Fast Delivery",
              description:
                "Books delivered to your door in 3-5 business days. Free shipping on orders over ₦50,000.",
            },
          ].map(({ icon, title, description }) => (
            <div
              key={title}
              className="text-center p-8 rounded-card-lg border border-brand-100 hover:border-brand-300 hover:shadow-soft transition-all"
            >
      ...
      <section className="bg-gradient-to-r from-brand-600 to-brand-700 py-16">
        <div className="max-w-3xl mx-auto text-center px-4">
          ...
          <p className="text-brand-200 mb-8">
            Join thousands of happy readers who&apos;ve found their favourites
            with us.
          </p>
          <Link
            href="/shop"
            className="inline-block px-10 py-4 bg-white text-brand-700 font-semibold rounded-pill hover:bg-brand-50 transition-colors shadow-lg"
          >
            Shop All Books
          </Link>
        </div>
