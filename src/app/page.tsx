...
const GENRES = [
  ...
  { name: 'Science Fiction', emoji: '🚀', color: 'from-indigo-100 to-indigo-50' },
...
export default async function HomePage() {
  const [featuredBooks, bestsellers] = await Promise.all([getFeaturedBooks(), getBestsellers()]);
  ...
  return (
    <main className="min-h-screen bg-surface">
          ...
          {[
            {
              ...
              description: 'Pre-loved books at a fraction of the original price. Knowledge should be accessible to everyone.',
            },
            {
              icon: (
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              description: 'Every book is reviewed for condition and accurately described. No surprises when it arrives.',
            },
            {
              icon: (
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              title: 'Fast Delivery',
              description: 'Books delivered to your door in 3-5 business days. Free shipping on orders over ₦50,000.',
            },
      ...
      {/* Footer */}
      <footer className="bg-brand-950 text-brand-400 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                {['All Books', 'Bestsellers', 'New Arrivals', 'Fiction', 'Non-Fiction'].map(l => (
                  <li key={l}><Link href="/shop" className="hover:text-white transition-colors">{l}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="text-white text-sm font-medium mb-4">Account</h4>
              <ul className="space-y-2 text-sm">
                {['Sign In', 'Register', 'My Orders', 'My Profile'].map(l => (
                  <li key={l}><Link href="/auth/signin" className="hover:text-white transition-colors">{l}</Link></li>
                ))}
              </ul>
            </div>
            <div>
